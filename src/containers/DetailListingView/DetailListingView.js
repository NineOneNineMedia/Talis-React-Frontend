...
export default function DetailListingView(props) {
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://127.0.0.1:8000/api/listings/listing/${listingID}`)
      .then((res) => {
        setListing(res.data);
        ...
        <Grid container direction="row" align="center">
          <Grid item direction="column" xs={12} md={7} align="start">
            <Grid item xs={12}>
              ...
              <Typography variant="p">{listing.property_address}</Typography>
            </Grid>
            <Grid item className={classes.marginT} xs={12}>
              <Typography variant="h6">Property Description</Typography>
              <Typography variant="p">{listing.description}</Typography>
            </Grid>
            <Grid
              container
