...
function ProfileView(props) {
    ...
    const getUserProfile = async () => {
        await axios.get(`http://127.0.0.1:8000/api/profiles/profile/${props.userId}`).then((res) => {
...
const mapStateToProps = (state) => {
    return { isAuthenticated: state.token !== null, userId: state.userId, token: state.token }
}
